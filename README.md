...
Ensure you have Python 3.10+ installed, then install the required ML libraries:
 
```bash
pip install -r requirements.txt
```
 
> **Note:** If you don't have a `requirements.txt`, run:
> ```bash
> pip install streamlit torch numpy transformers scikit-learn pandas datasets
> ```
 
### 3. Obtain the AI Model (Heavy Weights)
 
Because Git does not support large binary files, the trained `model.safetensors` file is not hosted in this repository.
...
├── app.py
├── requirements.txt
├── securechat_model_v2_final/   ← place model folder here
│   ├── config.json
│   ├── model.safetensors
...
| 📱 Model Compression | Converting the PyTorch `.safetensors` model to TensorFlow Lite (`.tflite`) for mobile deployment |
| ⚙️ Hyperparameter Tuning | Improving the F1-Macro score of the current DistilBERT model |
...
| 💡 Concept Contributors | Karthik & Prathamesh |
 
---
 
> 🛡️ **Thank you for exploring the SecureChat Prototype!**
>
> *"Privacy is not an option, and it shouldn't be the price we accept for just getting on the internet."*
> **— Gary Kovacs**
>
> Feel free to reach out, fork the repo, and contribute to making on-device safety a reality.
